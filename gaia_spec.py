...
def getGaiaInfo():
    ...
    resol_bp_I = scipy.interpolate.UnivariateSpline(wave_bp, gau_bp, ext=3,s=0)
    resol_rp_I = scipy.interpolate.UnivariateSpline(wave_rp, gau_rp, ext=3,s=0)
...
#        print(xind.sum())
        #pos = np.nonzero(xind)[0]
        #wht = #scipy.stats.norm(lam[i], gau[i]).pdf(lam[pos])
        pos=slice(x1,x2)
...
def get_bp_rp(filename):
    ...
    info = cache.info
    dat, hdr = pyfits.getdata(
        filename,
        header=True)
    wc = pywcs.WCS(hdr)
    pix = np.arange(len(dat))
    lam = wc.all_pix2world(pix, 0)[0] / 10  # nm
    dat = dat.astype(np.float64) / 6.6260755e-27 / 2.99792458e10 * lam *1e-7 * (
